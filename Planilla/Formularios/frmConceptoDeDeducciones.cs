...
{
    public partial class frmConceptoDeDeducciones : Form
    {
        public frmConceptoDeDeducciones()
        {
            InitializeComponent();
        }
        ...
        public ConceptoDeDeduccionEN[] oConceptoDeDeduccion = new ConceptoDeDeduccionEN[0];
        ...
        private void AgregarRegistroAlDTUsuario()
        {
            ...
            if (oConceptoDeDeduccion.Length > 0)
            {
        ...
        private string WhereDinamico()
        {
            ...
            {
                Where += string.Format(" and IdConceptoDeDeduccion like '%{0}%' ", txtIdentificador.Text.Trim());
            }
            if (Controles.IsNullOEmptyElControl(chkConcepto) == false && Controles.IsNullOEmptyElControl(txtConcepto) == false)
            {
                Where += string.Format(" and ConceptoDeDeduccion like '%{0}%' ", txtConcepto.Text.Trim());
            }
        ...
        private void FormatearDGV()
        {
            try
            {
                ...
                string OcultarColumnas = "IdConceptoDeDeduccion";
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "ConceptoDeDeduccion");
                            }
                            if (oFormato != null)
                            {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oConceptoDeDeduccion, a);

                            oConceptoDeDeduccion[a - 1] = new ConceptoDeDeduccionEN();
                            oConceptoDeDeduccion[a - 1].IdConceptoDeDeduccion = Convert.ToInt32(Fila.Cells["ID"].Value);
                            oConceptoDeDeduccion[a - 1].ConceptoDeDeduccion = Fila.Cells["ConceptoDeDeduccion"].Value.ToString();
            ...
            if (ActivarFiltros == true)
            {
                ...
                foreach (DataGridViewRow Fila in dgvLista.Rows)
                {
                    if (Convert.ToBoolean(Fila.Cells["Seleccionar"].Value) == true)
                    {
                        ...
                        oConceptoDeDeduccion[a - 1] = new ConceptoDeDeduccionEN();
                        oConceptoDeDeduccion[a - 1].IdConceptoDeDeduccion = Convert.ToInt32(Fila.Cells["IdConceptoDeDeduccion"].Value);
                        oConceptoDeDeduccion[a - 1].ConceptoDeDeduccion = Fila.Cells["ConceptoDeDeduccion"].Value.ToString();
        ...
        private void frmConceptoDeDeducciones_KeyUp(object sender, KeyEventArgs e)
        {
