...
namespace Planilla.Formularios
{
    public partial class frmContrato : Form
    {
        ...
        private string Nombre_Entidad_Privilegio = "Contrato";
        private string Nombre_Entidad = "Administrador de Contratos";
        private string Nombre_Llave_Primaria = "IdContrato";
        ...
        public ContratoEN[] oContrato = new ContratoEN[0];
        ...
        private string WhereDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkTipoContrato) == false && Controles.IsNullOEmptyElControl(txtTipoContrato) == false)
            {
                Where += string.Format(" and TipoDeContrato like '%{0}%' ", txtTipoContrato.Text.Trim());
            }
            if(Controles.IsNullOEmptyElControl(chkNumeroContrato) == false && Controles.IsNullOEmptyElControl(txtNumeroContrato) == false)
            {
                Where += string.Format(" and NumeroDeContrato like '%{0}%'", txtNumeroContrato.Text.Trim());
            }
        ...
        private void FormatearDGV()
        {
            try
            {
                ...
                string OcultarColumnas = "IdContrato, IdEmpleado";
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "NumeroDeContrato");
                            }
                            if (oFormato != null)
                            {
        ...
        private void frmContrato_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oContrato, a);

                            oContrato[a - 1] = new ContratoEN();
                            oContrato[a - 1].IdContrato = Convert.ToInt32(Fila.Cells["ID"].Value);
                            oContrato[a - 1].TipoDeContrato = Fila.Cells["Tipo De Contrato"].Value.ToString();
                            oContrato[a - 1].FechaDeInicio = Convert.ToDateTime(Fila.Cells["FechaDeInicio"].Value.ToString());
                            oContrato[a - 1].FechaDeFin = Convert.ToDateTime(Fila.Cells["FechaDeFin"].Value.ToString());
                            oContrato[a - 1].NumeroDeContrato = Convert.ToInt32(Fila.Cells["Numero De Contrato"].Value.ToString());
                            oContrato[a - 1].oEmpladoEN.Nombre = Fila.Cells["Empleado"].Value.ToString();
                        }
                    }
                }
            }
        ...
        private void frmContrato_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        {
            if (Controles.IsNullOEmptyElControl(txtTipoContrato))
            {
                chkTipoContrato.CheckState = CheckState.Unchecked;
            }
            else { chkTipoContrato.CheckState = CheckState.Checked; }

            if (chkTipoContrato.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LlenarListado();
            }
        }

        private void txtNumeroContrato_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtNumeroContrato))
            {
                chkNumeroContrato.CheckState = CheckState.Unchecked;
            }
            else { chkNumeroContrato.CheckState = CheckState.Checked; }

            if (chkNumeroContrato.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LlenarListado();
            }
        }
